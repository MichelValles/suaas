...
export function LaunchPanel({
  ...
  function launch() {
    ...
    const ids = [...selected];
    const estimateSec = Math.ceil((ids.length / 5) * stepCount * 5);
    setProgress(
      `Lanzando run · ${ids.length} ${ids.length === 1 ? "perfil" : "perfiles"} × ${stepCount} pasos. Estimado ~${estimateSec}s.`,
    );
    startSubmit(async () => {
      try {
        const res = await fetch("/api/runs/funnel", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ funnelId, profileIds: ids }),
        });
        ...
        setProgress(null);
        router.push(`/experiments/funnel/${json.runId}`);
      } catch (e) {
