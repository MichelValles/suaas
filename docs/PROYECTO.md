...
22. `0022_tiktok_ads.sql` (v0.55: canal TikTok; check de `strategy` con los 3 formatos `tiktok_*`, CTAs de TikTok en el check de `cta`, y de forma defensiva `channel_spec` + `company_name` 75c por si la 0021 no consta) **← pendiente de aplicar**

Las 20 primeras constan aplicadas en `suaas_migrations` (2026-06-11); la **0021 y la 0022 están pendientes de aplicar** (sin ellas las campañas de Google siguen funcionando, pero crear una campaña de Meta o de TikTok falla con el check de `strategy`). El script `scripts/apply-tiktok-setup.mjs` aplica las pendientes en orden leyendo `POSTGRES_URL_NON_POOLING` de `.env.local` y siembra la campaña TikTok de IVI sin lanzar runs; la alternativa es pegar los `.sql` en el SQL editor de Supabase. Convención desde la 0019: cada migración nueva inserta su propia fila al final. Tras cada `ALTER`, ejecutar `NOTIFY pgrst, 'reload schema';` en el SQL editor o esperar a que PostgREST refresque solo (lo hace cada ~10 min). `/diag` y `/api/diag` auditan el estado (sección «Tracking de migraciones» y campo `migrations_tracking`); el campo `pending_migrations` del JSON de `/api/diag` es el atajo para saber qué archivos `.sql` faltan por aplicar.
