...
| `brands` + `brand_documents` | **Cerebro** (v0.59, base de conocimiento de marca). `brands`: `name`, `description` (identidad reutilizable), `deleted_at`. `brand_documents`: `brand_id` (FK on delete cascade), `title`, `kind` (nota/brief/tono/producto/analytics/voc/informe), `content` (markdown, puede ser info privada). `lib/cerebro.ts`. El selector (`components/brand-picker.tsx` + `GET /api/brands`) vuelca `buildBrandContext` (descripción + documentos) en los campos de marca de los módulos. | Migración 0025. `deleted_at` en `brands` (papelera, tipo `brand`). |
