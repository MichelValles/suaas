...
- **Posicionamiento y documentación navegable (v0.77.2-v0.78.1)**:
  - **Redefinición al valor (`v0.77.2`)**: Gravity deja de ser «test de UX/CRO con perfiles calibrados» y pasa a «plataforma de **validación temprana**: mide la eficacia de un mensaje, una campaña o una decisión antes de comprometer tráfico real». Aplicado a copy visible (`/gravity/onboarding`) y fuentes canónicas (`CLAUDE.md`, `PROYECTO`, `ONBOARDING-SOCIOLOGO`). **Hero de la home** alineado al valor (`v0.78.0`).
  - **Visor de docs en la app (`v0.78.0`)**: `/docs` (índice) + `/docs/[slug]` (visor markdown con `react-markdown`, generación estática en build, tras auth). Las referencias `.md` del onboarding son enlaces (`DocLink` → `/docs/…`); los enlaces internos entre docs se reescriben a `/docs`.
  - **Fuera «usuarios sintéticos» (`v0.78.1`)**: perfiles calibrados enmarcados como **evolución del buyer persona**; doc base renombrado `CONOCIMIENTO-USUARIOS-SINTETICOS.md` → **`BASE-CONOCIMIENTO.md`** (34 refs en 17 ficheros). En citas académicas: paráfrasis + omisión de títulos con el término (autor/año/DOI/arXiv conservados). Excepciones a propósito: categoría de mercado en `Plan-venta`, entradas históricas del ROADMAP, y la regla de `CLAUDE.md` que cita el término para prohibirlo. Regla de nomenclatura actualizada. `PROYECTO`: «plataforma interna de Flat 101» → «para Flat 101» (`v0.78.2`).
  - **Onboarding enriquecido inline (`v0.78.3-v0.78.5`)**, dentro de los bloques existentes (sin secciones nuevas): §02 la mecánica conductual (qué hace COM-B, el vector Intent Momentum con int/dir/vel); §03 la toma de decisión óptima/fuga/repesca y de dónde sale el contexto («Gravity simula, no rastrea»); §05 la lógica fina del juez (voz + barreras, no campos mecánicos; fidelidad vs comprensión; `failure_mode`; razón→score); §07 tabla «qué se registra (observabilidad) / qué NO se rastrea» + nota «del coste al texto» (`gateway_usage` guarda conteos e identificadores, no el texto; la salida se cruza por `run_id`/`scope`/`profile_id` con la tabla de dominio; el prompt de entrada no se persiste).
- **Candidatos para retomar** (sin orden): validar el juez de calidad contra un **experto humano** (el gran pendiente de Fase 2, ver `ONBOARDING-SOCIOLOGO §6`); extender el juez a **funnel/pricing/copy**; atacar el hallazgo del **anclaje bajo del recall 5s** enriqueciendo `probeProfile`; explotar los perfiles IVI/Adeslas con experimentos reales; enlazar el visor de docs desde el sidebar/home si se quiere más visible. Bug latente conocido: el patrón `${color}22` (alfa hex tras `var()`) también está en componentes preexistentes (BehaviorCard de 5s/campañas): los tintes se descartan silenciosamente; migrar a `color-mix` si molesta.

- **Onboarding del sociólogo (v0.76-v0.77)**: doc `docs/ONBOARDING-SOCIOLOGO.md` (guión completo: estatuto, módulos, juez, y el protocolo de validación humana por niveles) y su **versión visual navegable** en `/gravity/onboarding` (índice pegajoso + scroll-spy, tarjetas, tablas, callouts). Revisada con un workflow adversarial (5 lentes); de ahí salió un bug de CSS real: **pegar alfa hex tras `var()`** (`var(--x)55`) no fusiona tokens y descarta el borde → usar `color-mix`. Corregido en la página y en los chips de calidad de 5s/campañas/momentum/intent (`v0.77.1`). Antipatrón documentado en `SISTEMA-DISENO`.
- **Juez de calidad en el módulo Intent / JTBD (v0.76.0)**: el JTBD (`intent_context`) no tiene «run», es un campo estático, así que el juez es **on-demand** desde la ficha (no persiste). La ficha ahora **muestra el JTBD** (antes solo en el editor) con un botón «Evaluar calidad» (`intent-quality-panel.tsx` + `intent-quality-actions.ts`, `judgeIntentQuality` en `lib/eval.ts`). Hallazgo (2026-07-22): el juez discrimina fórmula de manual (David, global 0,44, «genérico») vs JTBD anclado (Nuria, 0,87, anclaje 0,99), validando la refinación v0.74.4. Los hallazgos de fidelidad de toda la sesión están consolidados y fechados en `PERFILES-CALIBRADOS §8.7`.
- **Juez de calidad en Intent Momentum (v0.75.0)**: mismo patrón que 5s/campañas. `runMomentumQualitySample` juzga una muestra de reacciones al Trigger y adjunta la nota al resultado (`momentum_challenges.results`); la vista pinta panel + nota por perfil (`aggregateMomentumQuality`, `MomentumResultStored`). Se juzga la voz (relato + necesidad + frenos), no el plan mecánico. Verificado (IVI): naturalidad ~0,72 → ~0,88-0,91, overall hasta 0,95. Momentum deja de medirse a ojo. Pendiente Fase 2: validar contra experto humano; opcional Intent (JTBD) y funnel/pricing/copy.
- **Refinamiento de fidelidad en 5 módulos (v0.74.1-v0.74.5)**: subir el anclaje/naturalidad de las salidas de perfil, midiendo con el juez de calidad donde aplica. **Recall 5s** (`v0.74.1`): el probe pide atención selectiva y el juez recibe también las barreras (donde vive el filtro del perfil); anclaje ~0,2 → ~0,9. **Campañas** (`v0.74.2`): el razonamiento pasa de pro/contra ordenado a reacción espontánea; naturalidad ~0,71 → ~0,79-0,88, flag «robótico» casi eliminado. **Momentum** (`v0.74.3`): `intent_narrative` deja de ser un informe (el detalle va en los campos estructurados) y pasa a relato en primera persona. **Intent** (`v0.74.4`): el JTBD generado ancla en el backstory y usa la voz del perfil, sin fórmulas de manual. **GEO** (`v0.74.5`): guía las queries hacia preguntas conversacionales (la sonda es desnuda, sin persona; el lever es el realismo de la query, que la escribe el usuario). Todos verificados en producción.

- **Juez de calidad en los tests por lotes (v0.73-v0.74, Fase 2)**: el juez independiente del banco de pruebas (`/evaluacion`) se lleva a los runners. En **Claridad 5s** (`v0.73.0`) y en **Campañas** (`v0.74.0`), al cerrar el run se juzga una **muestra** (hasta 5) de las salidas de perfil con `judgeSimulationQuality`/`pickJudgeModel` (`lib/eval.ts`, juez de otra familia). La calidad es parte del resultado: panel «Calidad de la simulación» (global + 4 dimensiones) + nota con veredicto por respuesta muestreada; persistida en métricas `quality_*` y en el `meta` de las filas; coste al scope `quality_judge`. Clave metodológica (`v0.73.1`): el juez mide **fidelidad**, NO comprensión (que ya mide otro juez), para no confundir señales. Verificado en producción con runs reales. **Hallazgo**: el recall de 5s tiene anclaje bajo (factual, poco diferenciado por perfil) mientras las reacciones a campañas son de alta fidelidad (0,84 global) con el matiz «algo robótico». **Pendiente**: validar el juez contra un experto humano; opcionalmente extenderlo a funnel/pricing/copy.
...
- **Rediseño «oscuro editorial» en marcha (Fases 1-2, v0.64.0-v0.64.2)**: dirección validada en preview (registro Stripe/Linear, coherente con flat101.es, amarillo #F9CB0D). **Fase 1 (v0.64.0-v0.64.1)**: familia única **Hanken Grotesk** (fuera DM Serif Text y Nunito Sans), titulares a peso 700 con tracking negativo y **sin serif ni cursiva** (itálica reservada a citas). **Fase 2 (v0.64.2)**: superficies cálidas, el tema oscuro pasa a marfil `#f4f2ee`/`#f7f5f1` sobre negro cálido `#0c0b0a` con paneles `#1a1815`; se propaga solo por el canal `--fg` (`:root` + `.theme-dark-fixed`); el tema claro no se toca. **Fase 3 (en curso)**: `v0.64.3` retira los eyebrows mono del masthead (`PageHeading`); `v0.64.4` reescribe el chat del panel de perfil con registro WhatsApp (cabecera con avatar/estado, burbujas con cola, hora, ✓✓, «escribiendo…»), theme-aware por tokens `--chat-*`, preservando la lógica Talker-Reasoner; `v0.64.5` refina las cards (`feature-card`/`entity-card`: radio `lg`, sombra de elevación en hover, tinte al nuevo acento). Con esto la **Fase 3 queda cubierta** (masthead, chat, cards; las columnas ya existían donde aportan). **Fase 4 (v0.64.6)**: entrada orquestada de los bloques de página (`.app-shell-main > *`, fundido + subida escalonada por `nth-child`, `fill-mode: backwards`, reduced-motion cubierto por el bloque global). **Con esto el rediseño completa sus 4 fases.** `v0.64.7` cierra además la deuda del scroll horizontal (el `.tooltip-panel` de InfoTooltip pasa a `display:none` oculto, con `max-width` acotado y fundido moderno) y `v0.64.8` retira `/diseno-preview`. **El rollout del rediseño queda cerrado.** **Deuda anotada (ajena al rediseño)**: la página de perfil tiene scroll horizontal por un `.tooltip-panel` de InfoTooltip que desborda (COM-B); corregir en `InfoTooltip`. Detalle en `SISTEMA-DISENO.md` y `ROADMAP.md → v0.64.x`.
...
- **Documentación del motor (v0.61.15-16, v0.63.2)**: `docs/PERFILES-CALIBRADOS.md` (dossier del motor de perfiles con fundamento académico y citas archivo:línea, verificado a v0.63.3), `GRAVITY-MODEL.md` y `GRAVITY-MODEL-IVI.md` verificados contra el código, y `BASE-CONOCIMIENTO.md` reescrito con TODAS las fuentes rastreadas y las pruebas en vivo de guardarraíles, anti prompt injection y RAG (§8.3-8.4).
- **Decisión de producto (v0.63.1)**: la monitorización GEO periódica con cron queda descartada; la varianza inter-run se gestiona relanzando análisis a mano cuando haga falta.
- **Migraciones aplicadas: 0001 a 0029** (0025 a 0029 vía MCP de Supabase). El tracking vive en `suaas_migrations` y se ve en `/diag → Tracking de migraciones`.
...
- **Pendiente anotado (RESUELTO en v0.62.0)**: la contraseña de «Conceptos pendientes» (`/gravity`) estaba hardcodeada en cliente; ya movida al servidor (`POST /api/gravity/unlock` con `GRAVITY_PASSWORD` y `timingSafeEqual`, fallback histórico hasta que exista la env var).
...
- Ruta pública nueva `/onboard` para convertir humanos reales en gemelos digitales. Comparte el cuestionario desde la toolbar de `/profiles` (botón «Compartir cuestionario» con URL + QR).
