...
> Última actualización: 2026-07-20 tras v0.64.2 (rediseño «oscuro editorial»: Hanken Grotesk sin serif ni itálica, acento #F9CB0D, superficies cálidas).

## Estado actual (v0.64.2 desplegada)

- **Rediseño «oscuro editorial» en marcha (Fases 1-2, v0.64.0-v0.64.2)**: dirección validada en preview (registro Stripe/Linear, coherente con flat101.es, amarillo #F9CB0D). **Fase 1 (v0.64.0-v0.64.1)**: familia única **Hanken Grotesk** (fuera DM Serif Text y Nunito Sans), titulares a peso 700 con tracking negativo y **sin serif ni cursiva** (itálica reservada a citas). **Fase 2 (v0.64.2)**: superficies cálidas, el tema oscuro pasa a marfil `#f4f2ee`/`#f7f5f1` sobre negro cálido `#0c0b0a` con paneles `#1a1815`; se propaga solo por el canal `--fg` (`:root` + `.theme-dark-fixed`); el tema claro no se toca. Todo vía tokens en `globals.css`. **Pendiente del rollout**: Fase 3 composición por pantalla (masthead editorial, retirada de eyebrows, columnas, chat estilo WhatsApp en `chat-panel.tsx`); Fase 4 motion; refinamiento de componentes (`feature-card`/`entity-card` hover/hairlines); borrar `/diseno-preview`. Detalle en `SISTEMA-DISENO.md` y `ROADMAP.md → v0.64.x`.
