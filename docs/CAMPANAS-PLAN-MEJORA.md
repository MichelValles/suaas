...
### Consolidación de esquema con tracking de migraciones (0019) · M · necesita SQL · ✅ hecho (SQL aplicado por el operador el 2026-06-11; la poda de los fallbacks legacy pre-0011/pre-0013 y de los retries inertes se completó en v0.46.0)
...
### Runner tolerante a fallos con progreso observable y reanudación · L · sin SQL · ✅ hecho en v0.38.0
...
- **Cambiar el probe de Opus a Sonnet de inmediato**: a ciegas arriesgaba la calidad del scoring multimodal sin evidencia. **Resuelto en v0.47.2**: el incidente de la cuota (2026-06-11) zanjó la comparación; Opus 4.7 + `generateObject` + imágenes falla siempre el parse en el gateway (la contradicción documental de `five-second.ts:94-99` era real) y en texto puro el diferencial de calidad no justificaba el x2-x5 de precio. Todo el runner va en `DEFAULT_MODEL`.
