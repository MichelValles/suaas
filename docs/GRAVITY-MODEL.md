...
>
> **Relación entre ambos documentos**: los usuarios sintéticos son el *motor* (cómo se simula). El Gravity Model es el *marco estratégico* (qué se simula, por qué y en qué plano de la relación marca-usuario actúa cada simulación).
...
| GEO / nuevo estándar de búsqueda | GEO Tester: simula la respuesta de un buscador IA (Perplexity, Google AI Overview, ChatGPT Search) por segmento JTBD y mide `visibility_score`, `brand_position`, `recommendation_tone`, `key_claims`, `missing_attributes`. | `/geo`, `lib/geo.ts`, tabla `geo_analyses` (migración 0015) |
| Agentes sintéticos grounded (motor) | Perfiles con demografía + Big Five + COM-B + backstory, arquitectura Talker-Reasoner, negative prompting. | `lib/profiles.ts`, `lib/prompts.ts`, `lib/agents.ts`, base teórica en `CONOCIMIENTO-USUARIOS-SINTETICOS.md` |
