...
-- el producto bajo test en `product` (jsonb): title (150c), description
-- (5000c), price (ISO 4217), availability, brand (70c), gtin, condition.
-- El link del producto es final_url y la imagen principal va en creatives.

alter table campaigns add column if not exists product jsonb;

comment on column campaigns.product is
  'producto del feed para campañas shopping: {title, description, price, availability, brand?, gtin?, condition?}';
